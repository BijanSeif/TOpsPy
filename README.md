...
* **modeling** (SubPackage)
   ...
   - **MultiEl**       :  **bjm.MultiEl(4,5,5,elep)** #Define 5 elements with elep properties from node 4 to node 5. **bjm.MultiEl(4,5,5,elep,'Yes',1e9,10)** The last **'Yes'** cause that generated element be 2 head pinned using zero length Element with Rigid Stiffness equal to 1e9 or any other user value with NewMaterialTag equal to 10!. **(ATTENTION: it is very important that user don't define any previously used tag for NewMaterialTag)**
